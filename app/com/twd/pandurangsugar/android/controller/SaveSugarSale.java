 ...
 */
@WebServlet("/savesugarsale")
public class SaveSugarSale extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SaveSugarSale() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while (itr1.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if ((itemName != null) && (!itemName.trim().equals(""))) {
						File directory = new File(ConstantVeriables.baseFilePath+ConstantVeriables.sugar);
						if(!directory.exists()) {
							directory.mkdir();
						}
			...
			try(PrintWriter out=response.getWriter())
			{
				...
				if(action.equalsIgnoreCase("updatesugar"))
				{
