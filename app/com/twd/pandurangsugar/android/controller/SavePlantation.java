 ...
 */
@WebServlet("/savePlantation")
public class SavePlantation extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MainResponse mainresponse= new CanePlantationResponse();
		String json = null;
		String action = null;
		String imei = null;
		...
		try {
			...
			while (itr1.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if ((itemName != null) && (!itemName.trim().equals(""))) {
						File directory = new File(ConstantVeriables.baseFilePath+ConstantVeriables.plantation);
						if(!directory.exists()) {
							directory.mkdir();
						}
			...
			try(PrintWriter out=response.getWriter())
			{
				...
				CanePlantationServiceInterface dataLoad=new CanePlantationService();
				if(action.equalsIgnoreCase("savePlantation"))
				{
					JSONArray reqObj=new JSONArray(json);
					CanePlantationResponse saveData=dataLoad.savePlantation(reqObj,imei,ramdomstring,chit_boy_id,accessType,mainresponse);
					result=new JSONObject(saveData);
				}
				else if(action.equalsIgnoreCase("confirmPlantation"))
				{
					CanePlantationResponse saveData=dataLoad.updatePlantation(json,imei,ramdomstring,chit_boy_id,accessType,mainresponse);
					result=new JSONObject(saveData);
				}else
				{
