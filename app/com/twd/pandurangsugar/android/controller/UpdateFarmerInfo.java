 ...
 */
@WebServlet("/updateFarmerInfo")
public class UpdateFarmerInfo extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateFarmerInfo() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MainResponse mainresponse= new ActionResponse();
		String json = null;
		String action = null;
		String imei = null;
		...
		try {
			...
			while (itr1.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if ((itemName != null) && (!itemName.trim().equals(""))) {
						File directory = new File(ConstantVeriables.baseFilePath+ConstantVeriables.farmerInfo);
						if(!directory.exists()) {
							directory.mkdir();
						}
			...
			try(PrintWriter out=response.getWriter())
			{
				...
				if(action.equalsIgnoreCase("updateAadharByFarmerCode"))
				{
					JSONObject reqObj=new JSONObject(json);
					ActionResponse res=farmer.updateAadharByFarmerCode(reqObj,imei,ramdomstring,chit_boy_id,accessType,mainresponse);
					result=new JSONObject(res);
				}else if(action.equalsIgnoreCase("updateBankByFarmerCode"))
				{
					JSONObject reqObj=new JSONObject(json);
					ActionResponse res=farmer.updateBankByfarmerCode(reqObj,imei,ramdomstring,chit_boy_id,accessType,mainresponse);
					result=new JSONObject(res);
				}else
				{
